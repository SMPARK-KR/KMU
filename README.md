...
- **AI 유튜브 비전 분석 (Qwen2-VL)**: 유튜브 URL(Shorts 포함)을 실시간으로 임베딩 재생하며, 정지 화면을 캡처하여 최신 Vision-Language Model로 화면에 포함된 콘텐츠에 대한 상세한 한국어 분석 및 해설을 캔버스 크로핑 기반으로 제공합니다. (WebGPU 클라우드 오프 로딩 지원)
...
### 5. AI 유튜브 비전 분석 구동 팁 (중요)
- 본 기능은 크로스 도메인 `<iframe>` 간의 직접적인 화면 추출 보안 차단 정책을 우회하기 위해 **브라우저 Screen Capture API**를 활용합니다. `캡처 & 분석하기` 클릭 시 팝업되는 브라우저 창에서 **[이 탭 전체]** 혹은 해당 영역을 가볍게 한번 클릭승인해 주시면 AI 로컬 버퍼가 읽어들입니다.
- 연산 복잡도가 높으므로 **WebGPU**를 활성화한 최신 Chrome/Edge 브라우저로 구동하는 것을 강력하게 권장합니다.
